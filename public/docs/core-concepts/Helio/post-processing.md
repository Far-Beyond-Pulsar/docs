...
where $$$1$$ is the fully upsampled bloom pyramid result. The operation is purely additive — it never darkens the scene. This mirrors the physical process of light scatter in a lens: extra photons arrive at adjacent pixels, but none are removed from the source pixel.
...
where $$$1$$. Pixels whose combined horizontal and vertical luminance delta exceeds $$$1$$ are marked as edge pixels and receive anti-aliasing treatment in the subsequent passes. The threshold prevents noise and low-contrast texture detail from being misclassified as geometric edges.
...
where $$$1$$ cycles through a Halton or Hammersley low-discrepancy sequence. The factor of $$$1$$ converts from pixel-space to NDC half-steps. Over a sequence of frames these offsets tile the unit pixel area densely, so temporal accumulation converges to sub-pixel precision.
...
with $$$1$$ and $$$1$$ by default.

$$$1$$ is the **history weight** — how much of the accumulated past frames to keep. High $$$1$$ (→ 0.97) means more averaging across many frames, producing a smooth result but increasing ghosting behind fast-moving objects. Low $$$1$$ (→ 0.88) produces a sharper, more responsive result at the cost of more temporal noise on static geometry. The clamp prevents the blend factor from leaving the stable convergence range.
