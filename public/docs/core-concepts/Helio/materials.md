...
**Symbol guide:** $$$1$$ = view direction, $$$1$$ = light direction, $$$1$$ = half-vector, $$$1$$ = surface normal, $$$1$$ = roughness², $$$1$$ = base reflectance at normal incidence. The left term is the **Lambertian diffuse** lobe (divided by $$$1$$ for energy conservation, suppressed for metals via the $$$1$$ factor). The right term is the **Cook-Torrance specular** lobe built from three microfacet functions $$$1$$, $$$1$$, $$$1$$ in the numerator and a geometric normalisation factor $$$1$$ in the denominator.
...
$$D(\mathbf{n}, \mathbf{h}, \alpha) = \frac{\alpha^2}{\pi \left[(\mathbf{n} \cdot \mathbf{h})^2(\alpha^2 - 1) + 1\right]^2}$$
  ...
  $$$1$$; $$$1$$ is the cosine between the surface normal and the half-vector. The formula describes the statistical fraction of microfacet normals aligned with $$$1$$. High roughness ($$$1$$) flattens and widens the lobe; low roughness ($$$1$$) produces a tight, mirror-like highlight.
...
$$F_0 = \text{lerp}(0.04,\; \text{albedo},\; \text{metallic})$$
  ...
  $$$1$$ is the reflectance at normal incidence — $$$1$$ (4%) for dielectrics, equal to `albedo` for metals. As the view grazes the surface ($$$1$$) the $$$1$$ term approaches 1 and $$$1$$: everything becomes a mirror at 90°.
...
$$G_1(\mathbf{n}, \mathbf{x}, \alpha) = \frac{\mathbf{n} \cdot \mathbf{x}}{(\mathbf{n} \cdot \mathbf{x})(1 - k) + k}, \quad k = \frac{(\alpha + 1)^2}{8}$$
  ...
  The function is factored into two Schlick-GGX terms — one for the view direction ($$$1$$, self-shadowing) and one for the light direction ($$$1$$, masking). The remapping $$$1$$ reduces over-darkening on rough surfaces. $$$1$$ gives no shadowing; $$$1$$ gives full shadowing.
...
This **SNORM8** encoding saves 75% memory compared to a raw `f32` component (1 byte vs 4 bytes) with a worst-case angular error of $$$1$$ for unit normals — imperceptible in typical lighting calculations.
...
where $$$1$$ is the `bitangent_sign` stored in the `w` component of the packed tangent. The cross product $$$1$$ yields the vector perpendicular to both the normal and tangent; the sign $$$1$$ corrects for UV mirroring in left-handed UV spaces. This avoids storing the bitangent explicitly, saving 4 bytes per vertex with zero loss of precision.
...
When an object's world transform changes, both bounding volumes must be re-fitted in world space. Helio uses the **Arvo method** to transform an AABB without enumerating all eight corners. For a matrix $$$1$$ applied to an AABB $$$1$$, each output axis $$$1$$ is computed as:
...
Each output dimension depends only on the corresponding column of $$$1$$ and the input interval for that axis. This replaces 24 multiplications (8 corners × 3 components) with 9 min/max pairs (3 columns × 3 output axes), starting from the translation component of $$$1$$ as the initial min/max.
...
Given a combined view-projection matrix $$$1$$ with rows $$$1$$:
...
Each resulting $$$1$$ pair defines a half-space where $$$1$$ means the point $$$1$$ is on the inside (visible side) of the plane.
...
For a sphere with centre $$$1$$ and radius $$$1$$ against a plane $$$1$$:

$$\text{dist} = \hat{\mathbf{n}} \cdot \mathbf{c} + d$$
$$\text{outside if } \text{dist} < -r$$

If the signed distance from the plane to the sphere centre is less than $$$1$$, the entire sphere is on the negative (outside) side of the plane. The sphere is culled if this holds for **any** of the six frustum planes.
...
For a tighter cull, the AABB positive-vertex test checks the AABB corner most extreme in the plane's normal direction. For a plane normal $$$1$$, the positive vertex $$$1$$ is selected per axis:

$$p_x = \begin{cases} \text{aabb.max}_x & \text{if } n_x \geq 0 \\ \text{aabb.min}_x & \text{otherwise} \end{cases}$$

(identically for $$$1$$ and $$$1$$). The AABB is outside the plane if:
